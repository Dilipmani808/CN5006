...
const doc1 = new person_doc({ name: 'Aman shah',age:20,Gender:"Male",Salary:5000 }
);
...
doc1
 ...
 person_doc.find({})        //find all user

 .sort({Salary: 1})         //sort ascending by first name
 .select('name Salary age') //name and age only
...
.exec()
.then(docs => {
  console.log("showing age greater than 30", givenage); // Note: This log message is misleading.
  docs.forEach(function(Doc) {
    console.log(Doc.age, Doc.name);
